...
html:
  ...
  offline: true
  toc: true
title: PreCad coordinate file format ver 1.1 specification Rev.1

header-includes:
  - |
    <style>
    @media print {
      /* Force background colors for specific blocks */
      pre, code {
...
| asterisk| ＊ |
| circle| Circle |
| dot| Dot |Do not use size, use a setting for the size of dot. |
| plus| ＋ |
| square| Square |
| triangle| Triangle |
| x| X |
...
The alignment of the characters.   
`f` is 0: bottom left, 1: bottom center, 2: bottom right, 3: center left, 4: center, 5: center right, 6: top left, 7: top center, 8: top right.  
...
Draws a fan (Bézier curve) with center coordinates (x,y), radius radius, start angle start, corner width sweep, flatness flatness, and inclination angle.   
Can be filled.
...
- 2026/04/26
    - ver 1.1 Rev.1  
    - Corrections of typographical errors and formatting.
