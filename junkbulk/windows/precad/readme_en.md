...
3. The installer automatically retrieves and installs the following files. If you are installing in an environment where you cannot connect to the Internet, please download and run these files in advance.
    i. Microsoft Visual C++ Redistributable Package (64bit)<br>
    <https://aka.ms/vs/17/release/vc_redist.x64.exe>
    i. .NET Desktop Runtime 6.0.6(64bit)<br>
...
- Mail : <a href="mailto:ai@junkbulk.com?subject=About PreCad for Windows">ai@junkbulk.com</a>
...
Now you can install it for now.<br>
However, when I start PreCad, draw a line, and enlarge the screen, I notice that the line width is strange.<br>
So, select Run winconfig in Winetricks and add gdiplus from the libraries inside. This will make the line width normal.<br>
However, in the native version of gdiplus, if you do not properly specify the Japanese font, Japanese characters will be garbled (Japanese characters will also be garbled with this installer. In Windows, the characters will not be garbled because they will be displayed in an alternative font. The built-in version also doesn't seem to have garbled characters.I think you'll need to edit FontLink in the registry, but that seems difficult).<br>
If you don't have many Japanese fonts, run `winetricks allfonts` to install them (of course, you can select and install only Japanese fonts instead of all).<br>
...
- 2026/04/09 ver. 1.10.2.0
  - When saving DXF files, fills are now converted to SOLID.
  - When saving JWW files, the 4-point solid function is now used.
  - Circle solid support has been added for saving JWW files.
  - When saving DXF and JWW files, transparent colors will not be output.
  - ByLayer processing has been changed for saving and importing DXF files.
  - Layer display and print flags are now supported for saving and importing DXF files.
  - When saving DXF files, line widths are now output when available.
  - When importing DXF files, line widths are now used when available.
  - When saving DXF files, the color index has been corrected.
  - Fixed an issue with the ellipse radius when saving DXF files.

- 2026/03/28 ver. 1.10.1.0
  - Fixed an issue where SVG files had a BOM (Byte Order Mark).
  - Added UTF-8 support for DXF file writing and support for ellipse and polyline elements.
  - Added an auto mode for encoding when reading DXF files.
  - Added support for the slant angle of TEXT entity when importing DXF files.

- 2026/03/23 ver. 1.10.0.0
  - Supports file format v2.11.0.
  - Added linear gradient and radial gradient to fills.
  - Fixed behavior when pressing BS during numerical input.
  - Changed so that a menu is displayed when right-clicking the calculator button side of numerical input.
  - Fixed an issue where page numbers were not updated when dragging the page list.
  - Fixed an issue where the angle of elliptical arcs was reversed when saving as an SVG.
  - Fixed an issue where the dimension unit px could not be read when importing SVG.
  - Changed to use viewBox when importing SVGs.

- 2026/02/21 ver. 1.9.2.0
  - Fixed character spacing when importing PDF.
  - If the font name is empty, an alternative font will be used if an alternative font is enabled in the settings.
  - Removed encoding when writing DXF.
  - Changed the parameters for the dashed line when importing the PDF.
  - Fixed various file import plugins.
