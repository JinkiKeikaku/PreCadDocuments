# PreCad file format ver 2.3仕様書
## PreCadの特徴
- PreCadは2DCADです。複数のページを作成することができます。
- 各ページには通常のレイヤーの他にシートがあります。シートは縮尺を設定できます。レイヤーもシートも必ず一つあります。
...
- mediaフォルダは画像などを同梱する時に使います。画像はこのフォルダに保存する以外に各図面ファイル内に同梱することもできます。
- インデックスファイル及び図面ファイルはテキストファイルで文字コードはUTF8です。ファイルフォーマットはタグパラメータ形式を使用しています。タグパラメータ形式はタグの次にパラメータを()で囲み記述します。パラメータは数値、文字列、タグパラメータを複数含めることができます。またパラメータがないタグも存在します。タグの出現順は一部（セクション、ページ、レイヤ、シート、図形の順序など）を除き自由です。
...
```
- タグパラメータ形式では数値は整数（Int）もしくは実数（Double）でタグごとに判断します。
- タグ名は大文字小文字を区別します。
...
## インデクスファイル
インデックスファイルのファイル名は`index`です。このファイルにページの情報と全体の設定を記述します。
...
settings(
 ...
 grid(p0(0.0 0.0)spacing(10.0)div(10)isScaled(0))
...
- マイナーバージョンは要素追加、タグ名の変更・削除などで上がります。この変更で新バージョンのファイルを旧バージョンで読むと情報の欠落などが起こるかもしれません。必要であれば警告を表示します。
- 読み込み時、マイナーバージョンにより処理を変えることを推奨しません。マイナーバージョンはあくまでも参考とし、追加されたタグ（もしくは廃止されたタグ）をもとに読み込みを行うべきです。
- リビジョンはその他軽微な変更で上がります（タグ名のtypo修正など）。
...
settings(
 pageIndex(0)   
 paper(size(420 297))
 grid(p0(0 0)spacing(10)div(5)isScaled(0))
 printInfo(printPaperSize(420.0 297.0)printCenter(0.0 0.0)printScale(2.0))
)
```
#### pageIndex : Int
...
- 図形には線種など共通する属性があります。
- 省略時の値が書かれていないパラメータも省略できますが値は未定義です。
#### 図形属性
<a id="lineStyle"></a>
###### lineStyle（線スタイル属性）
...
### バージョンによる違い
- ファイルバージョンは文字列でversionタグを使いますが、旧バージョンでは```fileversion(version : Int)```を使いメジャーバージョンのみでした。
...
- バージョン2.3.0からImageのsrcタグにファイル名を追加、圧縮ファイル内のmediaフォルダ内に画像を保存します。それ以前はimageタグでbase64で画像のバイナリを同梱していました。

### 短縮形について
タグ名は内部で使っている変数名を元に決めました。そのため比較的文字数が多くなってしまいました。
タグ名が多少長くてもCPUは十分速くファイルは圧縮するためにそれほど問題にならないと思っていました。実際、ファイルサイズの増加はそれほどでもなく速度も（普通のファイルなら）問題になりませんでした。
